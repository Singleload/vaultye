              ...
              data.recentActivity.map((point, i) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className={`w-2 h-12 rounded-full ${point.priority === 'CRITICAL' || point.priority === 'HIGH' ? 'bg-red-500' :
                        point.status === 'NEW' ? 'bg-blue-500' : 'bg-indigo-500'
                      }`}></div>
                    <div>
                      <h4 className="font-semibold text-slate-800">{point.title}</h4>
                      <p className="text-sm text-slate-500">System: {point.system.name} • {new Date(point.createdAt).toLocaleDateString()}</p>
                    </div>
                  </div>
                  <span className="px-3 py-1 text-xs font-medium rounded-full bg-slate-100 text-slate-600 lowercase">
                    {point.status}
                  </span>
                </motion.div>
              ))
            ...
            {data.upcomingMeetings.length === 0 ? (
              ...
              data.upcomingMeetings.map((meeting, i) => (
                <motion.div
                  ...
                  className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 relative overflow-hidden cursor-pointer"
                >
