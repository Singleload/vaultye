        ...
        {isOpen && (
          ...
          >
            <button 
              ...
              className="w-full text-left px-4 py-3 hover:bg-slate-50 flex items-center gap-2 text-sm text-slate-600 font-medium"
            >
              {system.isArchived ? <Eye size={16}/> : <EyeOff size={16}/>}
              {system.isArchived ? 'Återställ' : 'Arkivera'}
            </button>
            <button 
              onClick={() => { onDelete(); setIsOpen(false); }}
              className="w-full text-left px-4 py-3 hover:bg-red-50 flex items-center gap-2 text-sm text-red-600 border-t border-slate-50 font-medium"
            >
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-md" onClick={onClose}>
      ...
      >
        ...
        <input 
          autoFocus
          className="w-full p-3 border border-slate-200 rounded-xl mb-6 outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent font-medium placeholder:text-slate-300"
        ...
        <div className="flex justify-end gap-3">
          <button onClick={onClose} className="px-5 py-3 text-slate-600 font-bold hover:bg-slate-50 rounded-xl transition-colors">Avbryt</button>
          <button 
            disabled={!isMatch}
            onClick={onConfirm}
            className="px-5 py-3 bg-red-600 text-white font-bold rounded-xl hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-red-200"
          >
    ...
    <div className="space-y-8 relative">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 mt-2 text-lg">Hantera dina förvaltningsobjekt och deras livscykel.</p>
        </div>
        
        <div className="flex gap-3">
          <button 
            onClick={() => setShowArchived(!showArchived)}
            className={`flex items-center gap-2 px-5 py-3 rounded-2xl font-bold transition-colors ${showArchived ? 'bg-indigo-50 text-indigo-700' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}`}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {systems?.map((system, index) => (
          <motion.div
            ...
            className={`bg-white rounded-[1.5rem] p-6 shadow-sm border cursor-pointer relative group flex flex-col justify-between h-64
          ...
          >
            <div>
              ...
              <h3 className="text-xl font-bold text-slate-800 mb-2 truncate pr-2">{system.name}</h3>
              <p className="text-slate-500 text-sm line-clamp-2 leading-relaxed font-medium">{system.description || "Ingen beskrivning"}</p>
            </div>
        ...
        {isCreateModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-md" onClick={() => setIsCreateModalOpen(false)}>
            <motion.div 
              ...
              className="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg p-8 max-h-[90vh] overflow-y-auto"
              onClick={e => e.stopPropagation()}
            >
              ...
              <form onSubmit={handleCreateSubmit} className="space-y-5">
                ...
                {/* GRUNDLÄGGANDE INFO */}
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-1.5">Systemnamn</label>
                  <input required name="name" className="w-full p-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none transition-all font-medium" placeholder="t.ex. HR-Portalen" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-1.5">Beskrivning</label>
                  <textarea name="description" rows="2" className="w-full p-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none transition-all font-medium" placeholder="Vad gör systemet?" />
                </div>

                {/* SYSTEMÄGARE (GRUPPERING) */}
                <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input required name="ownerName" className="w-full p-3 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Anna Andersson" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium mb-1 text-slate-600">AD-användarnamn</label>
                      <input name="ownerUsername" className="w-full p-3 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" placeholder="ann01and" />
                    </div>
                    <div className="col-span-2">
                      <label className="block text-xs font-medium mb-1 text-slate-600">Email</label>
                      <input required name="ownerEmail" className="w-full p-3 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" placeholder="anna@foretag.se" />
                    </div>
                  </div>
                </div>

                {/* SYSTEMFÖRVALTARE (GRUPPERING) */}
                <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100 space-y-3">
                  <h4 className="text-xs font-bold text-slate-500 uppercase flex items-center gap-2">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input required name="managerName" className="w-full p-3 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Dennis Enström" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium mb-1 text-slate-600">Ditt AD-användarnamn</label>
                      <input required name="managerUsername" className="w-full p-3 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" placeholder="ens01den" />
                    </div>
                  </div>
                </div>
                ...
                {/* RESURSGRUPP */}
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-1.5">Resursgrupp</label>
                  <input name="resourceGroup" className="w-full p-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none transition-all font-medium" placeholder="t.ex. Styrgruppen" />
                </div>
                
                {/* KNAPPAR */}
                <div className="flex justify-end gap-3 pt-4">
                  <button type="button" onClick={() => setIsCreateModalOpen(false)} className="px-5 py-3 text-slate-600 font-bold hover:bg-slate-50 rounded-xl transition-colors">Avbryt</button>
                  <button type="submit" className="px-6 py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 shadow-lg shadow-indigo-200 transition-all">Skapa System</button>
                </div>
              </form>
            </motion.div>
          </div>
